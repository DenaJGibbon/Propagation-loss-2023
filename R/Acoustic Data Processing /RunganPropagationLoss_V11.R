...
input.dir <- '/Users/denaclink/Desktop/RStudio Projects/Propagation-Loss-2020-2021/RunganAll'
...
for(b in 1:length(file.name.index)){tryCatch({
  ...
  # Check to make sure number of selections matches the template
    ...
    for(d in 1:nrow(singleplayback.df)){
      ...
      OrangYN <- SelectionIDsRungan[d,]$Orang
      ...
      write.csv(BackgroundNoiseRemovedDFRungan,'BackgroundNoiseRemovedDFRunganFeb2023.csv',row.names = F)
    }
...
write.csv(CombinedDF,'BackgroundNoiseRemovedDFRunganFeb2023SignalWithNoiseCombinedDF.csv',row.names = F)
