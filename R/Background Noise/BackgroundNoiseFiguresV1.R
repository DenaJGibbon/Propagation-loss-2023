...
RunganNoisePlot <- ggplot(RunganNoiseMedian,aes(center.freq,ambient.noise,
  ...
  scale_color_manual(values= matlab::jet.colors(length(unique(RunganNoiseMedian$time))) )+ theme_bw()+ylim(25,55)
...
MaliauNoisePlot <- ggplot(MaliauNoiseMedian,aes(center.freq,ambient.noise,
  ...
  scale_color_manual(values= matlab::jet.colors(length(unique(MaliauNoise$time))) )+ theme_bw()+ylim(25,55)
...
ggplot(CombinedNoiseMedian,aes(center.freq,ambient.noise,
  ...
  theme_bw()+ylab(expression(paste('Ambient sound level (dB re 20', mu,'Pa)',sep=' ')))+xlab('Center Frequency (Hz)')# + facet_wrap(~time)
...
ggplot(CombinedNoiseMedian,aes(center.freq,ambient.noise,
  ...
  xlab('Center Frequency (Hz)')+xlim(150,2000)+ylim(25,45)#+ facet_wrap(~time)
