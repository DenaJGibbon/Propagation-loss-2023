...
# Split the "Sound.Type" column into components based on the underscore ('_') delimiter
# Extract the second component (column 2) of the split string
# 'n=3' ensures that the string is split into at most 3 parts
TempSoundType <- 
  str_split_fixed(SelectionIDsRungan$Sound.Type, pattern = '_',n=3)[,2]

# Extract the first two characters (substring of length 2) from each element of 'TempSoundType'
TempSoundType <- substr(TempSoundType,start = 1,stop=2)

# Identify rows to remove where the sound type is NOT "Hf", "Ha", "Pm", or "Pw" (i.e., sweeps and phasers) 
PulsesToRemove <- which(TempSoundType!="Hf" & TempSoundType!="Ha"
                        & TempSoundType!="Pm" & TempSoundType!="Pw" )

# Create a sequence from 1 to the total number of rows in the data frame
PlaybackSeq <- seq(1,nrow(SelectionIDsRungan),1)

# Remove rows from the data frame where 'PulsesToRemove' indicates invalid sound types
...
# This file contains predicted sound propagation loss (in dB) for different sites
PredictedSpreading <- read.csv("data/Predicted_dB_Spherical.csv")

# Filter the 'PredictedSpreading' data frame to include only rows where the "Site" column is equal to 'Munkgu' (i.e., Rungan)
PredictedSpreadingRungan <- subset(PredictedSpreading,Site=='Munkgu')

# Read in data file
# Each row corresponds to a playback
rungan_data <- read.csv("data/PropLoss_test_7Jul22v2.csv")

# Create a contingency table (cross-tabulation) of the two columns: "Loc_Name" and "Habitat" that shows the number of rows for each combination of "Loc_Name" and "Habitat"
table(rungan_data$Loc_Name,rungan_data$Habitat)

# Combine the 'date' and 'time' columns from the 'RunganDF' data frame into the format "date_time"
date.time.combo <- paste(RunganDF$date,RunganDF$time,sep='_')

# Find all unique combinations of the 'date_time' strings (removes duplicate entries)
unique.date.time.combo <- unique(date.time.combo)

# Extract all unique values from the 'Loc_Name' column of the 'rungan_data' data frame and create a vector of unique location names
Loc_Name.index <- 
  unique(rungan_data$Loc_Name)

# Remove unwanted location names ('char1', 'char2', 'char3') from 'Loc_Name.index' 
Loc_Name.index  <- Loc_Name.index[- which(Loc_Name.index %in% c('char1','char2','char3'))]

# Create a data frame that links a set of characterization numbers to corresponding recorder IDs
...
# variable z iterates over integers from 1 to 7, corresponding to values in Loc_Name.index. This loop processes each unique location sequentially. tryCatch catches and outputs errors without breaking the entire loop
for(z in c(1:7)  ) { tryCatch({ 
  
  # For the location being processed (z in Loc_Name.index), a subset of RunganDF is created where Loc_Name == Loc_Name.index[z].
  # Subset data frame to focus on unique date/time
  temp.playback <- subset(RunganDF,Loc_Name==Loc_Name.index[z])
  
  
  # Create an index for each unique sound in the playback
